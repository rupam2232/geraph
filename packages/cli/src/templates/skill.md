...
If your environment has configured the Geraph MCP Server, you will natively see these tools available: `search_graph`, `get_node`, `get_neighbors`, `shortest_path`, and `scan_graph`. **You MUST prioritize using these native tools** over running terminal CLI commands, as they are faster, strictly typed, and not subject to terminal output bugs. If they are available, use them and skip the CLI steps below.
...
### Step 3: Use `node` and `neighbors` for Deep Inspection
Once you have found the exact Node ID or exact Symbol Name from Step 2 (or if the user explicitly provided one), use `node` to fetch its metadata and `neighbors` to fetch its full dependencies.

**MCP Tools:** `get_node` (for node metadata) or `get_neighbors` (for tracing edges and dependencies).\
**CLI Command:**
```bash
geraph node '<symbol_or_id>' [--type <type>] [--source <file>]
geraph neighbors '<symbol_or_id>' [--type <type>] [--source <file>] [--page <number>] [--limit <number>]
```
- **Inputs**: The exact Node ID (e.g., `src/auth/session.ts::ValidateToken`) or the exact symbol name (e.g., `ValidateToken`). If you are exploring a specific file, you can query its filepath directly (e.g., `geraph node 'src/app.ts' --type file` or `get_neighbors` with `node_id: "src/app.ts"`).
- **Options**:
  ...
  - `--page <number>` / `--limit <number>` (For `neighbors`): Paginate through the `incoming` and `outgoing` edges. For MCP tools, pass `page` and `limit` arguments.
...
When you run `neighbors` (or `get_neighbors`), you will see `incoming` and `outgoing` edges.
...
- **Pathing**: If you want to know how two nodes are connected, use the `shortest_path` MCP tool. (CLI Alternative: `geraph path '<source>' '<target>'`)
...
1. **NEVER read `.geraph/graph.json` directly.** It is massive and will overflow your context window. Always use the CLI or MCP tools.
2. **NO EXTRA FLAGS**: There are NO extra or hidden options. DO NOT hallucinate flags like `--dfs`. Only use `--type`, `--source`, `--page`, and `--limit`.
...
4. **TERMINAL OUTPUT BUG**: If your terminal fails to capture stdout (e.g., it hangs or returns "No output"), DO NOT GIVE UP. Redirect the output to a file: `geraph neighbors 'Symbol' > .geraph/out.json`, then read that file.
...
**`node` output**: Returns a detailed object with the `target` node:
...
**`neighbors` output**: Returns a detailed object with `target` (minimal identity details to confirm resolution), `incoming`, `outgoing`, and `meta`:
...
When you `node` or `neighbors` a symbol name (e.g., `geraph node 'userState'`), Geraph resolves it in this strict order:
